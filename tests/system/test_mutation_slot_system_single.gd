...
func _load_mutation_slot_script() -> GDScript:
	return load("res://scripts/mutation_slot.gd") as GDScript
...
func _make_inventory_instance() -> Object:
	var script: GDScript = load("res://scripts/mutation_inventory.gd") as GDScript
	if script == null:
		return null
	return script.new()
...
func test_mutation_slot_script_exists_and_is_pure_logic() -> void:
	...
	if script == null:
		_fail(
			...
			"res://scripts/mutation_slot.gd not found; implement MutationSlot per SLOT-1 spec"
		)
		return
