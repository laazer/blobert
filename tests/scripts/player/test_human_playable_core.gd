...
#
# CHECKPOINT [HP-001]:
#   Assumption: The main playable scene is res://scenes/levels/test_movement.tscn
...
func _load_main_scene() -> Node:
	...
	if packed == null:
		_fail("scene_load_main", "could not load res://scenes/levels/test_movement.tscn")
		return null
...
func test_chunk_scene_has_visual_child_at_origin() -> void:
	var packed: PackedScene = load("res://scenes/chunk/chunk.tscn") as PackedScene
	if packed == null:
		_fail("visual_chunk_scene_load", "could not load res://scenes/chunk/chunk.tscn")
		return
...
const HUMAN_PLAYABILITY_CHECKLIST: Array = [
	{
		...
		"steps": [
			"Open res://scenes/levels/test_movement.tscn in the Godot editor.",
