...
func _load_core_scene() -> Node:
	...
	if packed == null:
		_fail("hud_core_scene_load", "could not load res://scenes/levels/test_movement.tscn")
		return null
...
func _load_infection_scene() -> Node:
	...
	if packed == null:
		_fail("hud_infection_scene_load", "could not load res://scenes/levels/test_infection_loop.tscn")
		return null
	var inst: Node = packed.instantiate()
	if inst == null:
