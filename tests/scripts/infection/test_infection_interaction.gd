...
func _load_mutation_inventory_script() -> GDScript:
	return load("res://scripts/mutation/mutation_inventory.gd") as GDScript


# Load InfectionAbsorbResolver script; returns null if not found.
func _load_absorb_resolver_script() -> GDScript:
	return load("res://scripts/infection/infection_absorb_resolver.gd") as GDScript
...
func test_mutation_inventory_script_exists() -> void:
	...
	if script != null:
		...
		_fail(
			...
			"res://scripts/mutation/mutation_inventory.gd not found; implement MutationInventory (grant, has, get_granted_count) per ticket"
		)
...
func test_infection_absorb_resolver_script_exists() -> void:
	...
	if script != null:
		...
		_fail(
			...
			"res://scripts/infection/infection_absorb_resolver.gd not found; implement InfectionAbsorbResolver (can_absorb, resolve_absorb) per ticket"
		)
...
func test_infection_loop_scene_loads() -> void:
	...
	if packed == null:
		_fail("ii_scene_loads", "could not load res://scenes/levels/test_infection_loop.tscn")
		return
