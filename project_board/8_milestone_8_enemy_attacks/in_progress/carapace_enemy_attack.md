...
| 2 | Primary behavioral tests (deterministic, headless) | Test Designer Agent | Spec from task 1 | `tests/scripts/enemy/test_carapace_husk_attack.gd` (or agreed path); named cases (T-CEA-*) | Task 1 | Tests fail until implementation matches spec; cover range gate, telegraph min duration, charge axis, stop conditions, cooldown, deceleration | SceneTree limits documented per `test_attack_telegraph_system` patterns if needed |
| 3 | Adversarial tests | Test Breaker Agent | Spec + primary tests | `tests/scripts/enemy/test_carapace_husk_attack_adversarial.gd` | Tasks 1–2 | Edge cases (re-entry, zero dir, concurrent states, knockback/damage bounds) | Conservative defaults per checkpoint protocol |
...
```json
{
  ...
  "dependencies_completed": ["hitbox_and_damage_system", "attack_telegraph_system"]
}
```
...
## Reason
Planning complete: execution plan and checkpoint assumptions logged; specification can define telegraph duration strategy, damage/knockback, and charge termination without ambiguity.
