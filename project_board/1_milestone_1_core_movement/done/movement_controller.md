...
4. `project.godot` at the repo root must not interfere with `3D-Platformer-Kit/project.godot`.
5. No new files may be placed inside `3D-Platformer-Kit/` — it is read-only reference material.
...
- **Constraints:**
  - Must not reference any path inside `3D-Platformer-Kit/`.
  - Must not be placed inside `3D-Platformer-Kit/`.
...
- AC-10.4: `[input]` section contains the `move_left` action with deadzone `0.5` and two events:
  ...
  - The exact serialized object string format follows the Godot 4 project.godot convention as shown in the reference file at `3D-Platformer-Kit/project.godot` lines 37–41, extended with a second event entry.
...
- Risk: The `InputEventKey` serialized object string in Godot 4.3's `project.godot` format is sensitive to field ordering and values. The implementer should use the reference `3D-Platformer-Kit/project.godot` as the exact template for the object string structure, replacing only `physical_keycode` and `unicode` values. The Left Arrow physical keycode in Godot 4 is `4194319`; the Right Arrow is `4194321`. These are the `KEY_LEFT` and `KEY_RIGHT` constants from Godot's `Key` enum.
