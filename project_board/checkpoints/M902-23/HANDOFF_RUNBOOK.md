...
5. Only advance the ticket when both gates PASS in **blocking** mode. Orchestrators must use `run_workflow_transition_gates.py` (see `mandatory_workflow_gates_v1.md`).
...
Orchestrators: **do not** pass `--mode shadow` or `handoff_optional`. Tests only: `BLOBERT_ALLOW_GATE_OPT_OUT=1` with `handoff_optional` via `gate_runner.py`.
