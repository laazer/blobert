...
def create_cylinder(location=(0, 0, 0), scale=(1, 1, 1), vertices=8, depth=2.0):
    ...
    bpy.ops.mesh.primitive_cylinder_add(vertices=vertices, depth=depth, location=location, scale=scale)
...
def bind_mesh_to_armature(mesh_obj, armature):
    ...
    try:
        ...
        if len(mesh_obj.vertex_groups) > 0:
            print(f"✅ Auto weights successful: {len(mesh_obj.vertex_groups)} vertex groups created")
            return mesh_obj
        else:
...
def identify_vertex_body_part(vertex_pos, mesh_center, mesh_obj):
    ...
    if (rel_z > 0.6 and  # Upper part
        (rel_x > 0.7 or rel_x < 0.3) and  # Front area  
        vertex_pos.length < 0.3):  # Small objects
        return 'eye'
