...
class TestCreateMouthMeshShapeDispatch:
    ...
    def test_fang_calls_create_cone(self) -> None:
        ...
        with patch.object(bu, "create_cone", return_value=fake) as mock_cone:
            result = bu.create_mouth_mesh("fang", LOC, 1.0)
    ...
    def test_beak_calls_create_cone(self) -> None:
        ...
        with patch.object(bu, "create_cone", return_value=fake) as mock_cone:
            result = bu.create_mouth_mesh("beak", LOC, 1.0)
        assert result is fake
        mock_cone.assert_called_once()

    def test_flat_calls_create_box(self) -> None:
        fake = _fake_obj("flat_obj")
        with patch.object(bu, "create_box", return_value=fake) as mock_box:
            result = bu.create_mouth_mesh("flat", LOC, 1.0)
        assert result is fake
        mock_box.assert_called_once()

    def test_grimace_calls_create_cylinder(self) -> None:
        fake = _fake_obj("grimace_obj")
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            result = bu.create_mouth_mesh("grimace", LOC, 1.0)
        assert result is fake
    ...
    def test_smile_calls_create_cylinder(self) -> None:
        ...
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            result = bu.create_mouth_mesh("smile", LOC, 1.0)
        assert result is fake
        mock_cyl.assert_called_once()
    ...
    def test_unknown_shape_falls_back_to_cylinder(self) -> None:
        ...
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            result = bu.create_mouth_mesh("NOT_A_REAL_SHAPE", LOC, 1.0)
        assert result is fake
        mock_cyl.assert_called_once()
    ...
    def test_all_shapes_return_an_object(self, shape: str) -> None:
        ...
        with (
            patch.object(bu, "create_cone", return_value=fake),
            patch.object(bu, "create_cylinder", return_value=fake),
            patch.object(bu, "create_box", return_value=fake),
            patch.object(bu, "create_sphere", return_value=fake),
        ):
            result = bu.create_mouth_mesh(shape, LOC, 1.0)
        assert result is fake
    ...
    def test_scale_constants_used_for_fang(self) -> None:
        ...
        fake = _fake_obj()
        with patch.object(bu, "create_cone", return_value=fake) as mock_cone:
            bu.create_mouth_mesh("fang", LOC, 1.0)
        call_kw = mock_cone.call_args.kwargs
        sx, sy, sz = call_kw["scale"]
        assert sx == pytest.approx(bu._MOUTH_FANG_X_RATIO)
        assert sz == pytest.approx(bu._MOUTH_FANG_Z_RATIO)

    def test_scale_constants_used_for_smile(self) -> None:
        """Verify named scale constants are applied for smile shape."""
        fake = _fake_obj()
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            bu.create_mouth_mesh("smile", LOC, 1.0)
        call_kw = mock_cyl.call_args.kwargs
        sx, _sy, sz = call_kw["scale"]
        assert sx == pytest.approx(bu._MOUTH_SMILE_X_RATIO)
        assert sz == pytest.approx(bu._MOUTH_SMILE_Z_RATIO)
...
class TestCreateTailMeshShapeDispatch:
    ...
    def test_spike_calls_create_cone(self) -> None:
        ...
        with patch.object(bu, "create_cone", return_value=fake) as mock_cone:
            result = bu.create_tail_mesh("spike", 1.0, LOC)
        assert result is fake
        mock_cone.assert_called_once()
    ...
    def test_whip_calls_create_cylinder(self) -> None:
        ...
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            result = bu.create_tail_mesh("whip", 1.0, LOC)
        assert result is fake
        mock_cyl.assert_called_once()

    def test_club_calls_create_sphere(self) -> None:
        fake = _fake_obj("club_obj")
        with patch.object(bu, "create_sphere", return_value=fake) as mock_sphere:
            result = bu.create_tail_mesh("club", 1.0, LOC)
        assert result is fake
    ...
    def test_segmented_calls_create_cylinder(self) -> None:
        ...
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            result = bu.create_tail_mesh("segmented", 1.0, LOC)
        assert result is fake
        mock_cyl.assert_called_once()
    ...
    def test_curled_falls_back_to_create_sphere(self) -> None:
        ...
        with patch.object(bu, "create_sphere", return_value=fake) as mock_sphere:
            result = bu.create_tail_mesh("curled", 1.0, LOC)
        assert result is fake
        mock_sphere.assert_called_once()
    ...
    def test_unknown_shape_falls_back_to_sphere(self) -> None:
        ...
        fake = _fake_obj("fallback_obj")
        with patch.object(bu, "create_sphere", return_value=fake) as mock_sphere:
            result = bu.create_tail_mesh("UNKNOWN_SHAPE", 1.0, LOC)
        assert result is fake
        mock_sphere.assert_called_once()
    ...
    def test_all_shapes_return_an_object(self, shape: str) -> None:
        ...
        with (
            ...
            result = bu.create_tail_mesh(shape, 1.0, LOC)
        assert result is fake
    ...
    def test_length_scales_spike_cone(self) -> None:
        ...
        fake = _fake_obj()
        with patch.object(bu, "create_cone", return_value=fake) as mock_cone:
            bu.create_tail_mesh("spike", 2.0, LOC)
        call_kw = mock_cone.call_args.kwargs
        sx, sy, sz = call_kw["scale"]
    ...
    def test_whip_scale_constants_applied(self) -> None:
        ...
        fake = _fake_obj()
        with patch.object(bu, "create_cylinder", return_value=fake) as mock_cyl:
            bu.create_tail_mesh("whip", 1.0, LOC)
        call_kw = mock_cyl.call_args.kwargs
        sx, sy, _sz = call_kw["scale"]
        assert sx == pytest.approx(bu._TAIL_WHIP_XY_RATIO)
        assert sy == pytest.approx(bu._TAIL_WHIP_XY_RATIO)
