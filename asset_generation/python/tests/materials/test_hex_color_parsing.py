...
def testparse_hex_color_red():
    """Parse #ff0000 as red."""
    r, g, b, a = parse_hex_color("ff0000")
...
def testparse_hex_color_blue():
    """Parse #0000ff as blue."""
    r, g, b, a = parse_hex_color("0000ff")
...
def testrgba_from_hex_or_fallback_red():
    """Test rgba_from_hex_or_fallback with red hex."""
    fallback = (0.0, 0.0, 0.0, 1.0)
    result = rgba_from_hex_or_fallback("ff0000", fallback)
...
def testrgba_from_hex_or_fallback_uses_fallback_for_empty():
    """Test rgba_from_hex_or_fallback uses fallback for empty hex."""
    fallback = (0.5, 0.5, 0.5, 1.0)
    result = rgba_from_hex_or_fallback("", fallback)
    assert result == fallback, f"Should use fallback for empty hex, got {result}"


def testrgba_from_hex_or_fallback_sanitizes_input():
    """Test rgba_from_hex_or_fallback sanitizes input."""
    fallback = (0.0, 0.0, 0.0, 1.0)
    result = rgba_from_hex_or_fallback("#FF0000", fallback)
