...
class TestStripesTextureGenerator:
    ...
    def test_two_periods_produce_different_output(self) -> None:
        a = stripes_texture_generator(
            64, 8, "ff0000", "ffffff", 0.2
        )
        b = stripes_texture_generator(
    ...
    def test_stripe_width_clamps_at_minimum_boundary(self) -> None:
        ...
        a = stripes_texture_generator(64, 16, "ff0000", "ffffff", 0.0)
        b = stripes_texture_generator(64, 16, "ff0000", "ffffff", 0.05)
        assert a == b

    def test_stripe_width_clamps_at_maximum_boundary(self) -> None:
        """Values above maximum should clamp to the same output as 1.0."""
        a = stripes_texture_generator(64, 16, "ff0000", "ffffff", 9.0)
        b = stripes_texture_generator(64, 16, "ff0000", "ffffff", 1.0)
        assert a == b
    ...
    def test_presets_beachball_doplar_swirl_produce_distinct_output(self) -> None:
        ...
        pb = stripes_texture_generator(**base, stripe_preset="beachball")
        pd = stripes_texture_generator(**base, stripe_preset="doplar")
        ps = stripes_texture_generator(**base, stripe_preset="swirl")
        assert pb != pd
        assert pb != ps
        assert pd != ps
    ...
    def test_discrete_yaw_rotations_produce_different_textures(self) -> None:
        ...
        tex_0 = stripes_texture_generator(**base, rot_y_deg=0.0)
        tex_90 = stripes_texture_generator(**base, rot_y_deg=90.0)
        tex_180 = stripes_texture_generator(**base, rot_y_deg=180.0)
        tex_270 = stripes_texture_generator(**base, rot_y_deg=270.0)
    ...
    def test_non_discrete_angles_do_not_snap_to_90_multiples(self) -> None:
        """Projection-based yaw should preserve non-right-angle rotations."""
        base = dict(
            width=32,
            height=32,
            stripe_color_hex="ff0000",
        ...
        # Non-right-angle yaws should produce distinct patterns.
        tex_30 = stripes_texture_generator(**base, rot_y_deg=30.0)
        tex_45 = stripes_texture_generator(**base, rot_y_deg=45.0)
        tex_0 = stripes_texture_generator(**base, rot_y_deg=0.0)

        assert tex_30 != tex_0, "30° should differ from 0°"
        assert tex_45 != tex_0, "45° should differ from 0°"
        assert tex_30 != tex_45, "30° and 45° should differ"
    ...
    def test_rotation_wrapping_normalizes_equivalent_angles(self) -> None:
        ...
        tex_neg_90 = stripes_texture_generator(**base, rot_y_deg=-90.0)
        tex_270 = stripes_texture_generator(**base, rot_y_deg=270.0)
        tex_630 = stripes_texture_generator(**base, rot_y_deg=630.0)
    ...
    def test_large_texture_stress_is_deterministic(self) -> None:
        ...
        tex_a = stripes_texture_generator(**params)
        tex_b = stripes_texture_generator(**params)
        assert tex_a == tex_b
    ...
    def test_legacy_horizontal_vertical_map_to_presets(self) -> None:
        ...
        b = stripes_texture_generator(32, 32, "ff0000", "ffffff", 0.4, stripe_preset="doplar")
        assert a == b
        c = stripes_texture_generator(
            32, 32, "ff0000", "ffffff", 0.4, stripe_preset="vertical"
        )
        d = stripes_texture_generator(32, 32, "ff0000", "ffffff", 0.4, stripe_preset="beachball")
        assert c == d
    ...
    def testcrc32_ihdr_valid(self) -> None:
        ...
        assert stored_crc == crc32(ihdr_chunk)
    ...
    def testcrc32_idat_valid(self) -> None:
        ...
        while pos < len(png_data):
            ...
            if chunk_type == b"IDAT":
                ...
                assert stored_crc == crc32(idat_chunk)
                return
            pos += 12 + length
...
def teststripes_texture_generator_rejects_non_numeric_stripe_width() -> None:
...
def teststripes_texture_generator_rejects_non_int_width_or_height() -> None:
    with pytest.raises(TypeError, match="width must be an integer"):
        stripes_texture_generator(  # type: ignore[arg-type]
            width=32.0,
            height=32,
            stripe_color_hex="ff0000",
            bg_color_hex="ffffff",
...
def teststripes_texture_generator_rejects_bool_width_or_height() -> None:
    with pytest.raises(TypeError, match="width must be an integer"):
        stripes_texture_generator(  # type: ignore[arg-type]
            width=True,
            height=32,
...
def teststripes_texture_generator_rejects_non_positive_dimensions() -> None:
    with pytest.raises(ValueError, match="width and height must be positive"):
        stripes_texture_generator(0, 32, "ff0000", "ffffff", 0.5)
    with pytest.raises(ValueError, match="width and height must be positive"):
        stripes_texture_generator(32, -1, "ff0000", "ffffff", 0.5)


def teststripes_texture_generator_rejects_non_string_hex_colors() -> None:
...
def test_pitch_changes_texture_output() -> None:
    """Pitch (rot_x_deg) scales vertical component and must affect output."""
    base = dict(
        width=64,
        height=64,
        stripe_color_hex="ff0000",
    ...
    tex_0 = stripes_texture_generator(**base, rot_x_deg=0.0)
    tex_60 = stripes_texture_generator(**base, rot_x_deg=60.0)
    assert tex_0 != tex_60


def test_roll_changes_phase_offset() -> None:
    """Roll (rot_z_deg) shifts stripe phase along projection axis."""
    base = dict(
        width=64,
        height=64,
        stripe_color_hex="ff0000",
    ...
    tex_0 = stripes_texture_generator(**base, rot_z_deg=0.0)
    tex_180 = stripes_texture_generator(**base, rot_z_deg=180.0)
    assert tex_0 != tex_180
