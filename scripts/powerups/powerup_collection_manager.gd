...
const COLLECTION_RADIUS: float = 2.5

# Power-up duration constants
const SPEED_BOOST_DURATION: float = 15.0
const SHIELD_DURATION: float = 10.0

# Power-up value constants
const HEALTH_BOOST_VALUE: float = 25.0
const SPEED_BOOST_MULTIPLIER: float = 1.5
const SHIELD_VALUE: float = 1.0
const EXTRA_SLOT_VALUE: float = 1.0
...
func _physics_process(_delta: float) -> void:
	...
	for powerup in _active_powerups.duplicate():  # Duplicate to avoid modification during iteration
		...
		if distance < COLLECTION_RADIUS and player_controller != null:
...
func _get_duration_for_type(power_up_type: Type) -> float:
	match power_up_type:
		Type.SPEED_BOOST: return SPEED_BOOST_DURATION
		Type.SHIELD: return SHIELD_DURATION
		_: return 0.0

func _get_value_for_type(power_up_type: Type) -> float:
	match power_up_type:
		Type.HEALTH_BOOST: return HEALTH_BOOST_VALUE
		Type.SPEED_BOOST: return SPEED_BOOST_MULTIPLIER  # 50% speed increase
