...
# Timing constants
const PATROL_IDLE_WAIT: float = 1.0
const PATROL_DIRECTION_CHANGE_WAIT: float = 0.5
const ATTACK_COOLDOWN: float = 2.0
const PATROL_TIMER_MIN: float = 2.0
const PATROL_TIMER_MAX: float = 5.0

# Movement constants
const BASE_CHASE_SPEED: float = 3.0
const WEAKENED_CHASE_SPEED: float = 1.5
const DEFAULT_MOVE_SPEED: float = 2.0
const DIRECTION_THRESHOLD: float = 0.1
const VELOCITY_THRESHOLD: float = 0.1

# Detection constants
const RANDOM_THRESHOLD: float = 0.5
const WEAKENED_DETECTION_MULTIPLIER: float = 0.5
...
func _patrol(delta: float) -> void:
	match _current_patrol_state:
		PatrolState.IDLE:
			await get_tree().create_timer(PATROL_IDLE_WAIT).timeout
			_current_patrol_state = PatrolState.MOVING_LEFT if randf() > RANDOM_THRESHOLD else PatrolState.MOVING_RIGHT
		...
		PatrolState.MOVING_LEFT:
			...
			if _has_reached_boundary():
				...
				await get_tree().create_timer(PATROL_DIRECTION_CHANGE_WAIT).timeout
...
func _chase_player(delta: float) -> void:
	...
	if distance < ATTACK_RANGE and not _is_attack_cooldown_active():
		_trigger_attack()
...
func _trigger_attack() -> void:
	if attack_hitbox:
		...
		attack_cooldown_timer.start(ATTACK_COOLDOWN)  # 2 second cooldown
...
func _handle_weakened_state(_delta: float) -> void:
	...
	detection_range *= WEAKENED_DETECTION_MULTIPLIER
...
func _reset_patrol() -> void:
	_patrol_direction = Vector2.RIGHT if randf() > RANDOM_THRESHOLD else Vector2.LEFT
	_current_patrol_state = PatrolState.IDLE
	patrol_timer.start(rand_range(PATROL_TIMER_MIN, PATROL_TIMER_MAX))
